 ...
 *
 * This is the same as std::list except that it only uses preallocated memory and fail to grow when it runs out of it.
 * Use reserve( n ) to preallocate memory for n list items.
 * Then you can push_back() up to n items and they will be stored in the list
 * without the list allocating any new memory.
 * If you push_back() more items beyond n+1, they will not be added and push_back() will return false.
 *
 * You can increase the capacity of the list by calling reserve( m ) where m > n and continue push_back()-ing more items into it.
 *
 * BEWARE!
...
public:
	/** Constructor - n is the number of items to preallocate.
	*/
	rigid_list( size_t n = 0 ) : std::list<T>() { reserve( n ); };

	/** Preallocate memory for n list items.
	 * @param n - The number of items memory should be allocated for.
	 *
	 * If n is smaller than the current size of the list,
    * some items will be dropped from the end of the list.
	 */
	...
	/** Add an item to the end of the list.
 	 * @returns true if there was enough space to add the item, false otherwise.
	 */
	...
	*/
	...
	/** Insert a number of copies of an item at a given position.
	...
	* doesn't have enough free space to exchange for the new item, the operation fails.
	*
	* @returns true if splicing from the same list or if there was enough free elements to exchange for.
   ...
   */
	...
	*/
	...
	/** Erase several items from the list.
	* The capacity of the list is unaffected.
	*/
	...
	*/
	...
	/** Report the maximum number of items this list can currently hold.
	* The capacity can be changed by calling reserve().
	*/
	size_t capacity() { return this->size() + free_list.size(); };

	/** Report the number of items that can still be added to this list before its capacity is filled up.
	*/
